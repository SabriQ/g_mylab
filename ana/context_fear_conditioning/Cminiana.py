...
class MiniAna():
    ...
    def cellids_shockresponse(self,idx_accepted,df,corrected_ms_ts,normalization=True,shock_seconds=2,pre_shock_seconds=10,post_shock_seconds=60,shock_start_seconds=[]):
        """
        idx_accepted: all cell ids
        df: any dataframe contains the idx_accepted cells
        Normalization: (mean-min)/(max-min),  default True.
        Standarization: (x-mean)/std, default False
        fps: how many frames in 1 seconds
        shock_start_frame: the frame num of shock start
        ...
        shock_start_frameNums = [find_close_fast(corrected_ms_ts,i) for i shock_start_seconds ]

        baselines=[]
        postshocks_withshock=[]
        postshocks_withoutshock=[]
        for i in shock_start_frameNums：
            if i-pre_shock_seconds*fps < 0:
            ...
            if i+post_shock_seconds*fps+1 > total_length:
                print("post_shock_seconds set too long")
                end1 = total_length
            else:
                end1 = i+post_shock_seconds*fps+1
            postshock_withshock= df[idx_accepted].loc[i:end1]

            if i+(post_shock_seconds+shock_seconds)*fps+1 > total_length:
                print("post_shock_seconds set too long")
                end2 = total_length
