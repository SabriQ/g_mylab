...
del ms_load
#%% 丢弃前面4个block
sigraw = sigraw[sum([len(i) for i in del_ms_ts]):,:]
sigdeconvolved = sigdeconvolved[sum([len(i) for i in del_ms_ts]):,:]
CaTraces = CaTraces[sum([len(i) for i in del_ms_ts]):,:]
dff = dff[sum([len(i) for i in del_ms_ts]):,:]
ms_ts = keep_ms_ts
...
for aligned_behaveblock,contextcoord, blockname, in_context_msblock in zip(aligned_behaveblocks,contextcoords,blocknames, in_context_msblocks):
